...
class PostRepositoryImpl(private val dao: PostDao) : PostRepository {
    ...
    override suspend fun saveWithAttachment(post: Post, upload: MediaUpload) {
        try {
            ...
            val postWithAttachment = post.copy(attachment = Attachment(media?.id.toString(), AttachmentType.IMAGE))
    ...
    override suspend fun signIn(login: String, password: String): AuthState {
        return withContext(Dispatchers.IO) {
            try {
                val response = Api.service.updateUser(login, password) // Вызов метода API для авторизации
                if (!response.isSuccessful) {
                    throw ApiError(response.code(), response.message())
                }

                val auth = response.body() ?: throw ApiError(response.code(), response.message())
                delay(10_000L)
                AuthState(auth.id, auth.token)
            } catch (e: IOException) {
                throw e
